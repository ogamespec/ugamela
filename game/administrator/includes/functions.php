		...
		switch($f["fleet_mission"]){
			...
			case 1:{
				if($f['fleet_start_time']<=time()){
					...
					$deb = "Bu Kordinattaki Harabede {$zlom["metal"]} metal ve {$zlom["krysztal"]} Kristal bulunuyor.";
					...
					if($debris < 100000){
						...
						$szansa = rand(1, 100);
						$szanmoon = "Ay Olusma Sansi $szansa2 % ";
					}
					...
					if(($szansa > 0) and ($szansa <= $szansa2) and $galenemyrow['id_luna'] == 0){
						...
						$raport = "<center><table><tr><td>Savas ".date("r",$f["fleet_start_time"])." filolar::<br>";
						includeLang('tech');
						global $lang;
						$zniszczony = false;
						foreach ($dane_do_rw as $a => $b){
							$raport .= "<table border=1 width=100%><tr><th><br><center>Saldiran {$atakujacy_user["username"]} ({$f['fleet_start_galaxy']}:{$f['fleet_start_system']}:{$f['fleet_start_planet']})<br>Silahlanma: ".(100 + ($tech_atakujacy["military_tech"] * 10))."% Savunma: ".(100 + ($tech_atakujacy["defence_tech"] * 10))."% Korunma: ".(100 + ($tech_atakujacy["shield_tech"] * 10))."% <table border=1>";
							if ($b["atakujacy"]["ilosc"] > 0){
								$raport1 = "<tr><th>Tur</th>";
								$raport2 = "<tr><th>Il.</th>";
								$raport3 = "<tr><th>Silahlanma:</th>";
								$raport4 = "<tr><th>Savunma</th>";
								$raport5 = "<tr><th>Korunma</th>";
							...
							$raport .= "<table border=1 width=100%><tr><th><br><center>Saldirilan {$wrog_user["username"]} ({$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']})<br>Silahlanma: ".(100 + ($tech_wrog["military_tech"] * 10))."% Savunma: ".(100 + ($tech_wrog["defence_tech"] * 10))."% Korunma: ".(100 + ($tech_wrog["shield_tech"] * 10))."% <table border=1>";
							if ($b["wrog"]["ilosc"] > 0){
								$raport1 = "<tr><th>Tur</th>";
								$raport2 = "<tr><th>Il.</th>";
								$raport3 = "<tr><th>Silahlanma:</th>";
							...
							$raport .= "</table></center></th></tr></table>";
							if ($zniszczony == false){
								$raport .= "<br><center>Saldiran Filo Butun Gucuyle {$b["atakujacy"]["atak"]} vurdu.Saldirilanin Korunmasi {$b["wrog"]["tarcza"]} kadarini etkisiz kildi<br>
									Korunan Taraf Saldiriya Gecti ve {$b["wrog"]["atak"]} kadar vurdu.Saldiranin Korunmasi  {$b["atakujacy"]["tarcza"]} kadarini etkisiz kildi.</center>";
							}
						}

						switch ($wygrana) {
							case "a":
								$raport .= "<p>SALDIRAN TARAF KAZANDI!<br>Ganimet:<br>Metal: {$ladownia["metal"]}<br>Kristal: {$ladownia["krysztal"]}<br>Deuterium: {$ladownia["deuter"]}<br><p><br>{$deb} <br>{$szanmoon} <br>$powtal<br>";
								break;
							case "r":
								$raport .= "<p>BERABERE!<br><p><br>{$deb} <br>{$szanmoon} <br>$powtal<br>";
								break;
							case "w":
								$raport .= "<p>SALDIRIYA UGRAYAN TARAF KAZANDI!<br><p><br>{$deb} <br>{$szanmoon} <br>$powtal<br>";
								doquery("DELETE FROM {{table}} WHERE fleet_id=".$f["fleet_id"],'fleets');
								break;
							default:
								break;
						}
						$raport .= "Simulasyon {$totaltime} saniyede yapildi.<br>Savas Raporu ve Savas Simulasyonu By OgameTr</table>";
						//echo $raport;
						$dpath = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
						...
						$raport = "<a class=\"thickbox\" href=\"rw.php?raport=".$rid."&keepThis=true&TB_iframe=true&height=400&width=500\"><font color=\"red\">Filo Raporu [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] (V:{$zlom["wrog"]},A:{$zlom["atakujacy"]})</font></a>";
						...
						doquery("INSERT INTO {{table}} SET 
							...
							`message_type`='0',
							`message_from`='Filo Lideri',
							`message_subject`='Savas',
							`message_text`='{$raport}'"
							,'messages');
...
//					$deuterium=$f['fleet_resource_deuterium'];
				...
				if ($f["fleet_mess"] != "1"){
					doquery("INSERT INTO {{table}} SET
						...
						`message_text`='Filo Hedefe Vardi $nazwatwojej [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] ve sukadar hammadde getirdi [Metal: {$f['fleet_resource_metal']} Kristal: {$f['fleet_resource_crystal']} Deuterium: {$f['fleet_resource_deuterium']}]'"
						,'messages');              
					doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
					doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
				}	
					if($f['fleet_end_time']<=time()){
						...
						if ($f["fleet_mess"] == "1"){
								doquery("INSERT INTO {{table}} SET
				                     ...
				                     `message_from`='Filo Lideri',
				                     `message_subject`='Transport Raporu',
				                     `message_text`='Filo Hedefe Vardi $nazwamojej [{$f['fleet_start_galaxy']}:{$f['fleet_start_system']}:{$f['fleet_start_planet']}] Fakat Hammmadde Getirmedi'"
				                     ,'messages');              
								doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
...
if($idwroga['fusion_plant'] > "0"){$fusion="<td>Fizyo Enerji Santrali</td><td>{$idwroga['fusion_plant']}</td>";}
...
if($idwroga['crystal_store'] > "0"){$mag_krysi="<td>Kristal Deposu</td><td>{$idwroga['crystal_store']}</td>";}
...
if($szpiegwrog['defence_tech'] > "0"){$obr_tech="<td>Savunma Teknigi</td><td>{$szpiegwrog['defence_tech']}</td></tr>";}
...
if($szpiegwrog['graviton_tech'] > "0"){$gra_tech="<td>Gravitasyon Tekngi</td><td>{$szpiegwrog['graviton_tech']}</td>";}
								...
								if($szansawzor >= $szansazest){
									$wiadja = "<font color=\"red\">Yok Edildi</font>";
									$wiadty = ",Filo Yok Edildi!";
									doquery("DELETE FROM {{table}} WHERE fleet_id=".$f["fleet_id"],'fleets');
									$zestrzelona = true;
								...
								$szansa=" <center> Yok Etme Sansi: $szansawzor% <br><b>$wiadja</b></center> </td> </tr>";
								$pT = ($pozW - $pozT);
								$pW = ($pozT - $pozW);
								if ($pozW > $pozT){
									$ST = ($LS - pow($pT,2));
								...
								if ($ST <= "1"){
									if ($f["fleet_mess"] != "1"){
										...
										doquery("INSERT INTO {{table}} SET
											...
											`message_text`='Diger Gezegenin Filosu {$messmoja['name']} [{$messmoja["galaxy"]}:{$messmoja["system"]}:{$messmoja["planet"]}] hedefe vardi: {$idwroga['name']} [{$idwroga["galaxy"]}:{$idwroga["system"]}:{$idwroga["planet"]}] $wiadty'"
											,'messages');              
										doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$idwrog}'",'users'); 
										doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
									}
								...
								if ($ST == "2"){
									if ($f["fleet_mess"] != "1"){
										...
										doquery("INSERT INTO {{table}} SET
											...
											`message_from`='Filo Lideri',
											`message_subject`='Filo (Casusluk)',
											`message_text`='Diger Gezegenin Filosu{$messmoja['name']} hedefe vardi {$idwroga['name']} $wiadty'"
											,'messages');              
										doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$idwrog}'",'users'); 
										doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
									}
				...
				if($f['fleet_start_time']<=time()){
					...
					if ($f["fleet_mess"] == "0"){
						...
						doquery("INSERT INTO {{table}} SET
		                     ...
		                     `message_text`='Filonuz Su Kadar Hammmaddeyi Geri Donusturdu: Metal:{$odzyskane["metal"]} Kryszta³:{$odzyskane["crystal"]}'"
		                     ,'messages');              
						doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
						doquery("UPDATE {{table}} SET fleet_mess='2' WHERE fleet_id=".$f["fleet_id"],'fleets');
						doquery("UPDATE {{table}} SET
			...
			case 9:{
				...
				if(make_planet($f['fleet_end_galaxy'],$f['fleet_end_system'],$f['fleet_end_planet'],$f['fleet_owner'])){
					doquery("INSERT INTO {{table}} SET 
						...
						`message_from`='Filo Lideri',
						`message_subject`='Kolonilesme',
						`message_text`='Gezegen [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] kolonilestirildi'"
						,'messages');
				}else{
					echo "error";
				}
