	...
	foreach($lang['tech'] as $i => $n){//investigacion
		...
		if($i > 201&&$i <= 399){
			...
			if(!is_tech_available($user,$planetrow,$i)){
				...
				//die($user[$resource[$i]]);
				$nivel = ($building_level == 0) ? "" : " ($building_level avaliable)";
				//Descripcion
				...
				*/
				...
				//Muestra la opci n a elegir para construir o ampliar un edificio
				if($is_buyeable){
					$tabindex++;
				...
				if(!$teching){
					...
					if($user[$resource[$i]] == 0 && $is_buyeable){
						...
						if($planetrow["b_building_id"] == 31 &&
							$game_config['allow_invetigate_while_lab_is_update']!=1)
						{
							$buildinglist .= "<font color=#FF0000>Research</font>";
						}else{
							$buildinglist .= "<a href=\"?mode=research&bau=$i\"><font color=#00FF00>Research</font></a>";
						}
							...
							$game_config['allow_invetigate_while_lab_is_update']!=1)
						{
							...
							$buildinglist .= "<font color=#FF0000>Update to <br> $nplus</font>";
						}else{
							$nplus = $user[$resource[$i]] + 1;
							$buildinglist .= "<a href=\"?mode=research&bau=$i\"><font color=#00FF00>Update <br>to level $nplus</font></a>";
						}
						...
						$nplus = $user[$resource[$i]] + 1;
						$buildinglist .= "<font color=#FF0000>Update to level <br>$nplus </font>";
					}
				}
				else{
...
case 'defense':{//----------------------------------------------------------
	...
	if($planetrow[$resource[21]] == 0){
					...
					*/
					...
					if($is_buyeable){
						if (($i==407 && $planetrow['small_protection_shield'] == "1") ||($i==407 && $planetrow['b_hangar_id'] == "407,1;")||(($i==408 && $planetrow['big_protection_shield'] == "1") ||($i==408 && $planetrow['b_hangar_id'] == "408,1;")))
						 ...
						 $tabindex++;
					    	 $is_buyeable = false;
						 $buildinglist .= "Kalkan Kubbesi Sadece 1 Kez Yapýlýr";
					    	}
						else{
						$tabindex++;
		...
		$buildinglist .= '</td></tr>
		<td class=c colspan=2 align=center><input type=submit value="Tamam">
...
if(in_array($bau,$reslist['build']) && is_tech_available($user,$planetrow,$bau) && is_buyable($user,$planetrow,@$bau)){
	...
	if($planetrow["field_current"] < get_max_field($planetrow) && $planetrow["b_building_id"] == 0){
		/*
...
elseif(in_array($unbau,$reslist['build'])&&$planetrow['b_building_id'] == $unbau){
	...
	$user['points_builds']-=$costs['metal']+$costs['crystal']+$costs['deuterium'];

//	$user['points_points']-=$costs['metal']+$costs['crystal']+$costs['deuterium'];
		...
		if(!is_tech_available($user,$planetrow,$i)){//:)
			...
			*/
			...
			if(!$building){
				...
				//Muestra la opcion a elegir para construir o ampliar un edificio
				}elseif($planetrow[$resource[$i]] == 0 && $planetrow["field_current"] < get_max_field($planetrow)  && $is_buyeable){
					$parse['click'] = "<a href=\"?bau=$i\"><font color=#00FF00>{$lang['Build']}</font></a>";
				}elseif($planetrow["field_current"] < get_max_field($planetrow) && $is_buyeable){
