...
//					$deuterium=$f['fleet_resource_deuterium'];
				...
				if ($f["fleet_mess"] != "1"){
					doquery("INSERT INTO {{table}} SET
						...
						`message_time`='".time()."',
						`message_type`='0',
						`message_from`='Fleet Update',
						`message_subject`='Transport Report',
						`message_text`='Fleet Missiong success $nazwatwojej [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] here are the results [Metal: {$f['fleet_resource_metal']} Kristal: {$f['fleet_resource_crystal']} Deuter: {$f['fleet_resource_deuterium']}]'"
						,'messages');              
					doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
					doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
				}	
