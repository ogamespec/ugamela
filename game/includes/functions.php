			...
			case 1:{
				if($f['fleet_start_time']<=time()){
					...
					if (!isset($pricelist[202]["sd"])){
						message("<font color=\"red\">Yeni vars.phpye sahip olmalisiniz </font>","error","fleet.".$phpEx,2);
					}
					...
					$deb = "At these coordinates float {$zlom["metal"]} metal and {$zlom["krysztal"]} Crystal .";
					...
					if($debris < 100000){
						...
						$szansa = rand(1, 100);
						$szanmoon = "Chance moon was created $szansa2 % ";
					}
					...
					if(($szansa > 0) and ($szansa <= $szansa2) and $galenemyrow['id_luna'] == 0){
						...
						$raport = "<center><table><tr><td>Battle Report  ".date("r",$f["fleet_start_time"])." .::<br>";
						includeLang('tech');
						global $lang;
						$zniszczony = false;
						foreach ($dane_do_rw as $a => $b){
							$raport .= "<table border=1 width=100%><tr><th><br><center>Attacking Player: {$atakujacy_user["username"]} ({$f['fleet_start_galaxy']}:{$f['fleet_start_system']}:{$f['fleet_start_planet']})<br>Weapons: ".(100 + ($tech_atakujacy["military_tech"] * 10))."% Armour: ".(100 + ($tech_atakujacy["defence_tech"] * 10))."% Shielding: ".(100 + ($tech_atakujacy["shield_tech"] * 10))."% <table border=1>";
							if ($b["atakujacy"]["ilosc"] > 0){
								$raport1 = "<tr><th>Type</th>";
								$raport2 = "<tr><th>No.</th>";
								$raport3 = "<tr><th>Weapons:</th>";
								$raport4 = "<tr><th>Armour</th>";
								$raport5 = "<tr><th>Shielding</th>";
								...
								$zniszczony = true;
								$raport .= "<br>Attacker Destroyed";
							}
							
							$raport .= "</table></center></th></tr></table>";

							$raport .= "<table border=1 width=100%><tr><th><br><center>Defending Player {$wrog_user["username"]} ({$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']})<br>Weapons: ".(100 + ($tech_wrog["military_tech"] * 10))."% Armour: ".(100 + ($tech_wrog["defence_tech"] * 10))."% Shielding: ".(100 + ($tech_wrog["shield_tech"] * 10))."% <table border=1>";
							if ($b["wrog"]["ilosc"] > 0){
								$raport1 = "<tr><th>Type</th>";
								$raport2 = "<tr><th>No.</th>";
								$raport3 = "<tr><th>Weapons:</th>";
							...
							$raport .= "</table></center></th></tr></table>";
							if ($zniszczony == false){
								$raport .= "<br><center>Attacking fleet fired {$b["atakujacy"]["atak"]} times and did {$b["wrog"]["tarcza"]} points of damage.<br>
									Defending fleet fires {$b["wrog"]["atak"]} times and does {$b["atakujacy"]["tarcza"]} points of damage.</center>";
							}
						}

						switch ($wygrana) {
							case "a":
								$raport .= "<p>You had won the battle!!<br>Spoils of War:<br>Metal: {$ladownia["metal"]}<br>Crystal: {$ladownia["krysztal"]}<br>Deuterium: {$ladownia["deuter"]}<br><p><br>{$deb} <br>{$szanmoon} <br>$powtal<br>";
								break;
							case "r":
								$raport .= "<p>There was a tie(draw) no one had won<br><p><br>{$deb} <br>{$szanmoon} <br>$powtal<br>";
								break;
							case "w":
								$raport .= "<p>You had lost the battle!<br><p><br>{$deb} <br>{$szanmoon} <br>$powtal<br>";
								doquery("DELETE FROM {{table}} WHERE fleet_id=".$f["fleet_id"],'fleets');
								break;
							default:
								break;
						}
						$raport .= "Battle report took {$totaltime} seconds to complete.<br></table>";
						//echo $raport;
						$dpath = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
						...
						doquery("INSERT INTO {{table}} SET 
							...
							,'rw');
						//$raport = "<a class=\"thickbox\" href=\"rw.php?raport=".$rid."&keepThis=true&TB_iframe=true&height=400&width=500\">Report</a>";
						$raport = "<a class=\"thickbox\" href=\"rw.php?raport=".$rid."&keepThis=true&TB_iframe=true&height=400&width=500\"><font color=\"white\">Fleet Report [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] (V:{$zlom["wrog"]},A:{$zlom["atakujacy"]})</font></a>";
...
//					$deuterium=$f['fleet_resource_deuterium'];
				...
				if ($f["fleet_mess"] != "1"){
					doquery("INSERT INTO {{table}} SET
						...
						`message_text`='Filo Target $nazwatwojej [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] ve sukadar hammadde getirdi [Metal: {$f['fleet_resource_metal']} Crystal: {$f['fleet_resource_crystal']} Deuterium: {$f['fleet_resource_deuterium']}]'"
						,'messages');              
					doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
					doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
				}	
					if($f['fleet_end_time']<=time()){
						...
						if ($f["fleet_mess"] == "1"){
								doquery("INSERT INTO {{table}} SET
				                     ...
				                     `message_from`='Fleet Leader',
				                     `message_subject`='Transport Raporu',
				                     `message_text`='Filo Target $nazwamojej [{$f['fleet_start_galaxy']}:{$f['fleet_start_system']}:{$f['fleet_start_planet']}] Fakat Hammmadde Getirmedi'"
				                     ,'messages');              
								doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
					...
					foreach($fleet as $a => $b){
						if($b != ''){
							...
							if ($a[0] == "210"){
								...
								$surka="<table width=440><tr><td class=c colspan=4> Casusluk: {$idwroga['name']}[{$idwroga['galaxy']}:{$idwroga['system']}:{$idwroga['planet']}]".gmdate("d-m-Y H:i:s",time()+2*60*60)."</td></tr><tr><td>Metal:</td><td>".pretty_number($idwroga['metal'])."</td><td>Crystal:</td><td>".pretty_number($idwroga['crystal'])."</td></tr> <tr><td>Deuterium:</td><td>".pretty_number($idwroga['deuterium'])."</td> <td>Enerji:</td><td>".pretty_number($idwroga['energy_max'])."</td></tr> </table>";
								
if($idwroga['small_ship_cargo'] > "0"){$mt="<td>Small Cargo Ship</td><td>{$idwroga['small_ship_cargo']}</td>";}
if($idwroga['big_ship_cargo'] > "0"){$dt="<td>Big Cargo Ship</td><td>{$idwroga['big_ship_cargo']}</td></tr>";}
if($idwroga['light_hunter'] > "0"){$lm="<td>Light Fighter</td><td>{$idwroga['light_hunter']}</td>";}
if($idwroga['heavy_hunter'] > "0"){$cm="<td>Heavy Fighter</td><td>{$idwroga['heavy_hunter']}</td></tr>";}
if($idwroga['crusher'] > "0"){$kr="<td>Cruiser</td><td>{$idwroga['crusher']}</td>";}
if($idwroga['battle_ship'] > "0"){$ow="<td>Battle Ship</td><td>{$idwroga['battle_ship']}</td></tr>";}
if($idwroga['colonizer'] > "0"){$colon="<td>Colonizer</td><td>{$idwroga['colonizer']}</td>";}
if($idwroga['recycler'] > "0"){$recek="<td>Recycler</td><td>{$idwroga['recycler']}</td></tr>";}
if($idwroga['spy_sonde'] > "0"){$spysonda="<td>Espionage Probe</td><td>{$idwroga['spy_sonde']}</td>";}
if($idwroga['bomber_ship'] > "0"){$bombo="<td>Bomber Ship</td><td>{$idwroga['bomber_ship']}</td></tr>";}
if($idwroga['solar_satelit'] > "0"){$satki="<td>Solar Satelite</td><td>{$idwroga['solar_satelit']}</td>";}
if($idwroga['destructor'] > "0"){$niszcz="<td>Destructor</td><td>{$idwroga['destructor']}</td></tr>";}
if($idwroga['dearth_star'] > "0"){$gwiazdeczka="<td>Death Star</td><td>{$idwroga['dearth_star']}</td>";}
if($idwroga['battle_ship'] > "0"){$panc="<td>Battle Ship</td><td>{$idwroga['battle_ship']}</td></tr>";}
								//if($idwroga['silo'] > "0"){$silos="<td>Silos Rakietowy</td><td>{$idwroga['silo']}</td></tr>";}
								
								$floty="<table width=440><tr><td class=c colspan=4> Casusluk: {$idwroga['name']}[{$idwroga['galaxy']}:{$idwroga['system']}:{$idwroga['planet']}]".gmdate("d-m-Y H:i:s",time()+2*60*60)."</td></tr><tr><td>Metal:</td><td>".pretty_number($idwroga['metal'])."</td><td>Crystal:</td><td>".pretty_number($idwroga['crystal'])."</td></tr> <tr><td>Deuterium:</td><td>".pretty_number($idwroga['deuterium'])."</td> <td>Enerji:</td><td>".pretty_number($idwroga['energy_max'])."</td></tr> </table><table width=440><tr><td class=c colspan=6>Filo</td></tr>$mt$dt$lm$cm$kr$ow$colon$recek$spysonda$bombo$satki$niszcz$gwiazdeczka$panc </table>";
								
if($idwroga['misil_launcher'] > "0"){$ml="<td>Rocket Launcher</td><td>{$idwroga['misil_launcher']}</td>";}
if($idwroga['small_laser'] > "0"){$sl="<td>Small Laser</td><td>{$idwroga['small_laser']}</td></tr>";}
if($idwroga['big_laser'] > "0"){$bl="<td>Big Laser</td><td>{$idwroga['big_laser']}</td>";}
if($idwroga['gauss_canyon'] > "0"){$gauss="<td>Gauss Cannon</td><td>{$idwroga['gauss_canyon']}</td></tr>";}
if($idwroga['ionic_canyon'] > "0"){$ionic="<td>Ionic Cannon</td><td>{$idwroga['ionic_canyon']}</td>";}
if($idwroga['buster_canyon'] > "0"){$buster="<td>Buster Cannon</td><td>{$idwroga['buster_canyon']}</td></tr>";}
if($idwroga['small_protection_shield'] > "0"){$mp="<td>Small Protection Shield</td><td>{$idwroga['small_protection_shield']}</td>";}
if($idwroga['big_protection_shield'] > "0"){$dp="<td>Big Protection Shield</td><td>{$idwroga['big_protection_shield']}</td>";}
								
								
								$obrona="<table width=440><tr><td class=c colspan=4> Espionage {$idwroga['name']}[{$idwroga['galaxy']}:{$idwroga['system']}:{$idwroga['planet']}]".gmdate("d-m-Y H:i:s",time()+2*60*60)."</td></tr><tr><td>Metal:</td><td>".pretty_number($idwroga['metal'])."</td><td>Crystal:</td><td>".pretty_number($idwroga['crystal'])."</td></tr> <tr><td>Deuterium:</td><td>".pretty_number($idwroga['deuterium'])."</td> <td>Enerji:</td><td>".pretty_number($idwroga['energy_max'])."</td></tr> </table><table width=440><tr><td class=c colspan=6>Filo</td></tr>$mt$dt$lm$cm$kr$ow$colon$recek$spysonda$bombo$satki$niszcz$gwiazdeczka$panc </table> <table width=440><tr><td class=c colspan=4>Savunma</td></tr>$ml$sl$bl$gauss$ionic$buster$mp$dp </table>";
								
if($idwroga['metal_mine'] > "0"){$kop_metal="<td>Metal Mine</td><td>{$idwroga['metal_mine']}</td>";}
if($idwroga['crystal_mine'] > "0"){$kop_krysia="<td>Crystal Mine</td><td>{$idwroga['crystal_mine']}</td>";}
if($idwroga['deuterium_sintetizer'] > "0"){$kop_deut="<td>Deuterium Mine</td><td>{$idwroga['deuterium_sintetizer']}</td></tr>";}
if($idwroga['solar_plant'] > "0"){$solar="<td>Solar Energy Plant</td><td>{$idwroga['solar_plant']}</td>";}
if($idwroga['fusion_plant'] > "0"){$fusion="<td>Fusion Plant</td><td>{$idwroga['fusion_plant']}</td>";}
if($idwroga['robot_factory'] > "0"){$robot="<td>Robot Factory</td><td>{$idwroga['robot_factory']}</td></tr>";}
if($idwroga['nano_factory'] > "0"){$nano="<td>Nano Factory</td><td>{$idwroga['nano_factory']}</td>";}
if($idwroga['hangar'] > "0"){$stocznia="<td>Hangar</td><td>{$idwroga['hangar']}</td>";}
if($idwroga['metal_store'] > "0"){$mag_mety="<td>Metal Storage</td><td>{$idwroga['metal_store']}</td></tr>";}
if($idwroga['crystal_store'] > "0"){$mag_krysi="<td>Crystal Storage</td><td>{$idwroga['crystal_store']}</td>";}
if($idwroga['deuterium_store'] > "0"){$mag_deut="<td>Deuterium Storage</td><td>{$idwroga['deuterium_store']}</td>";}
if($idwroga['laboratory'] > "0"){$lab="<td>Research Lab</td><td>{$idwroga['laboratory']}</td></tr>";}
if($idwroga['terraformer'] > "0"){$tetra="<td>Terraformer</td><td>{$idwroga['terraformer']}</td>";}
if($idwroga['ally_deposit'] > "0"){$allydepo="<td>Alliance Depot</td><td>{$idwroga['ally_deposit']}</td>";}
if($idwroga['silo'] > "0"){$silos="<td>Missile Silo</td><td>{$idwroga['silo']}</td></tr>";}
								
								$budynki="<table width=440><tr><td class=c colspan=4> Espionage: {$idwroga['name']}[{$idwroga['galaxy']}:{$idwroga['system']}:{$idwroga['planet']}]".gmdate("d-m-Y H:i:s",time()+2*60*60)."</td></tr><tr><td>Metal:</td><td>".pretty_number($idwroga['metal'])."</td><td>Crystal:</td><td>".pretty_number($idwroga['crystal'])."</td></tr> <tr><td>Deuter:</td><td>".pretty_number($idwroga['deuterium'])."</td> <td>Enerji:</td><td>".pretty_number($idwroga['energy_max'])."</td></tr> </table><table width=440><tr><td class=c colspan=6>Filo</td></tr>$mt$dt$lm$cm$kr$ow$colon$recek$spysonda$bombo$satki$niszcz$gwiazdeczka$panc </table> <table width=440><tr><td class=c colspan=4>Savunma</td></tr>$ml$sl$bl$gauss$ionic$buster$mp$dp </table> <table width=440><tr><td class=c colspan=6>Binalar</td></tr></tr>$kop_metal$kop_krysia$kop_deut$solar$fusion$robot$nano$stocznia$mag_mety$mag_krysi$mag_deut$lab$tetra$allydepo$silos</table>";
								
if($szpiegwrog['spy_tech'] > "0"){$spy_tech="<td>Espionage Technology</td><td>{$szpiegwrog['spy_tech']}</td>";}
if($szpiegwrog['computer_tech'] > "0"){$pc_tech="<td>Computer Technology</td><td>{$szpiegwrog['computer_tech']}</td></tr>";}
if($szpiegwrog['military_tech'] > "0"){$boj_tech="<td>Military Technology</td><td>{$szpiegwrog['military_tech']}</td>";}
if($szpiegwrog['defence_tech'] > "0"){$obr_tech="<td>Defence Technology</td><td>{$szpiegwrog['defence_tech']}</td></tr>";}
if($szpiegwrog['shield_tech'] > "0"){$op_tech="<td>Shield Technology</td><td>{$szpiegwrog['shield_tech']}</td>";}
if($szpiegwrog['energy_tech'] > "0"){$ene_tech="<td>Energy Technology</td><td>{$szpiegwrog['energy_tech']}</td></tr>";}
if($szpiegwrog['hyperspace_tech'] > "0"){$nadp_tech="<td>Hyperspace Technology</td><td>{$szpiegwrog['hyperspace_tech']}</td>";}
if($szpiegwrog['combustion_tech'] > "0"){$spal_tech="<td>Combuestion Technology</td><td>{$szpiegwrog['combustion_tech']}</td></tr>";}
if($szpiegwrog['impulse_motor_tech'] > "0"){$imp_tech="<td>Impulse Technology</td><td>{$szpiegwrog['impulse_motor_tech']}</td>";}
if($szpiegwrog['hyperspace_motor_tech'] > "0"){$napna_tech="<td>Hyperspace Technology</td><td>{$szpiegwrog['hyperspace_motor_tech']}</td></tr>";}
if($szpiegwrog['laser_tech'] > "0"){$las_tech="<td>Laser Technology</td><td>{$szpiegwrog['laser_tech']}</td>";}
if($szpiegwrog['ionic_tech'] > "0"){$jon_tech="<td>Ion Technology</td><td>{$szpiegwrog['ionic_tech']}</td></tr>";}
if($szpiegwrog['buster_tech'] > "0"){$plaz_tech="<td>Plasma Technology</td><td>{$szpiegwrog['buster_tech']}</td>";}
if($szpiegwrog['intergalactic_tech'] > "0"){$msbn_tech="<td>Intergalactic Technology</td><td>{$szpiegwrog['intergalactic_tech']}</td></tr>";}
if($szpiegwrog['graviton_tech'] > "0"){$gra_tech="<td>Graviton Technology</td><td>{$szpiegwrog['graviton_tech']}</td>";}
								
								$badania="<table width=440><tr><td class=c colspan=4> Espionage: {$idwroga['name']}[{$idwroga['galaxy']}:{$idwroga['system']}:{$idwroga['planet']}]".gmdate("d-m-Y H:i:s",time()+2*60*60)."</td></tr><tr><td>Metal:</td><td>".pretty_number($idwroga['metal'])."</td><td>Crystal:</td><td>".pretty_number($idwroga['crystal'])."</td></tr> <tr><td>Deuterium:</td><td>".pretty_number($idwroga['deuterium'])."</td> <td>Enerji:</td><td>".pretty_number($idwroga['energy_max'])."</td></tr> </table><table width=440><tr><td class=c colspan=6>Filo</td></tr>$mt$dt$lm$cm$kr$ow$colon$recek$spysonda$bombo$satki$niszcz$gwiazdeczka$panc </table> <table width=440><tr><td class=c colspan=4>Savunma</td></tr>$ml$sl$bl$gauss$ionic$buster$mp$dp </table> <table width=440><tr><td class=c colspan=6>Binalar</td></tr></tr>$kop_metal$kop_krysia$kop_deut$solar$fusion$robot$nano$stocznia$mag_mety$mag_krysi$mag_deut$lab$tetra$allydepo$silos</table><table width=440><tr><td class=c colspan=4>Arastirma   </td></tr></tr>$spy_tech$pc_tech$boj_tech$obr_tech$op_tech$ene_tech$nadp_tech$spal_tech$imp_tech$napna_tech$las_tech$jon_tech$plaz_tech$msbn_tech$gra_tech</table>";
								...
								if($szansawzor >= $szansazest){
									$wiadja = "<font color=\"red\">Fleet Destroyed</font>";
									$wiadty = ",Fleet destroyed in battle";
									doquery("DELETE FROM {{table}} WHERE fleet_id=".$f["fleet_id"],'fleets');
									$zestrzelona = true;
								} elseif($szansawzor < $szansazest){ 
									$wiadja = "<font color=\"lime\">Kurtuldu</font>";
									$wiadty = "";
								}
								$szansa=" <center> Fleet Lost: $szansawzor% <br><b>$wiadja</b></center> </td> </tr>";
								$pT = ($pozW - $pozT);
								$pW = ($pozT - $pozW);
								if ($pozW > $pozT){
									$ST = ($LS - pow($pT,2));
								...
								if ($ST <= "1"){
									if ($f["fleet_mess"] != "1"){
										doquery("INSERT INTO {{table}} SET
											...
											`message_type`='0',
											`message_from`='Fleet Leader',
											`message_subject`='Espionage Report',
										...
										doquery("INSERT INTO {{table}} SET
											...
											`message_type`='0',
											`message_from`='Fleet Leader',
											`message_subject`='Fleet (Espionage)',
											`message_text`='Another Planet Fleet{$messmoja['name']} [{$messmoja["galaxy"]}:{$messmoja["system"]}:{$messmoja["planet"]}] Target: {$idwroga['name']} [{$idwroga["galaxy"]}:{$idwroga["system"]}:{$idwroga["planet"]}] $wiadty'"
											,'messages');              
										doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$idwrog}'",'users'); 
										doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
									}
								...
								if ($ST == "2"){
									if ($f["fleet_mess"] != "1"){
										...
										doquery("INSERT INTO {{table}} SET
											...
											`message_from`='Fleet Leader',
											`message_subject`='Fleet (Espionage)',
											`message_text`='Diger Gezegenin Fleet{$messmoja['name']} Target {$idwroga['name']} $wiadty'"
											,'messages');              
										doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$idwrog}'",'users'); 
										doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
									}
								...
								if ($ST == "4" or $ST == "3"){
									...
									doquery("INSERT INTO {{table}} SET
										...
										`message_type`='0',
										`message_from`='Fleet Leader',
										`message_subject`='Filo (Casusluk)',
										`message_text`='Another Planet Fleet{$messmoja['name']} Target {$idwroga['name']} $wiadty'"
										,'messages');              
									doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$idwrog}'",'users'); 
									doquery("UPDATE {{table}} SET fleet_mess='1' WHERE fleet_id=".$f["fleet_id"],'fleets');
									}
				...
				if($f['fleet_start_time']<=time()){
					...
					if ($f["fleet_mess"] == "0"){
						...
						doquery("INSERT INTO {{table}} SET
		                     ...
		                     `message_type`='0',
		                     `message_from`='Fleet Leader',
		                     `message_subject`='Geri Donusum Raporu',
		                     `message_text`='Fleet has returned from a harvest: Metal:{$odzyskane["metal"]} Kryszta³:{$odzyskane["crystal"]}'"
		                     ,'messages');              
						doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
						doquery("UPDATE {{table}} SET fleet_mess='2' WHERE fleet_id=".$f["fleet_id"],'fleets');
						doquery("UPDATE {{table}} SET
			...
			case 9:{
				...
				doquery("INSERT INTO {{table}} SET
					...
					`message_text`='Gezegen Kolonilestirme [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] imkansiz,zaten maximum gezegene sahipsiniz..'"
					,'messages');              
				doquery("UPDATE {{table}} SET new_message=new_message+1 WHERE id='{$f['fleet_owner']}'",'users'); 
				 }else
				if(make_planet($f['fleet_end_galaxy'],$f['fleet_end_system'],$f['fleet_end_planet'],$f['fleet_owner'])){
					doquery("INSERT INTO {{table}} SET 
						...
						`message_from`='Fleet Leader',
						`message_subject`='Kolonilesme',
						`message_text`='Planet [{$f['fleet_end_galaxy']}:{$f['fleet_end_system']}:{$f['fleet_end_planet']}] colonized'"
						,'messages');
				}else{
					echo "error";
				}
