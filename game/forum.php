...
function showthread($forum, $start=0) {
	...
	while ($t = mysql_fetch_array($query)) {
		...
		if($i==0){$page .= "<tr><td class=c>Author</td><td class=c>Message</td></tr>"; $f=$t['forum_id'];}
...
function reply($topic) {
	...
	if(!$topic_id){message("El tema no existe","Forums","forum.php");}
    ...
    if(isset($_POST["post"])) {
		...
		*/
		...
		message("Your post has been submitted you will now be redirected to the category!","Forums","forum.php?t=$topic_id#last");
    }
...
function newthread($forum_id) {
	...
	if(!$forum_cat){message("El foro no existe","Forums","forum.php");}
	/*
	  Formulario para crear un Forums
	*/
    if(isset($_POST["post"])) {
		extract($_POST);
		/*
		...
		*/
		...
		message("El Forums se agrego correctamente. En unos instantes seras redirigido.","Forums","forum.php?f=$forum_id");
    }

	
	$page = $forum_cat["forum_name"];
	
	form($page,"Forums - ".$forum_cat['forum_name'],"forum.php?new=thread&f=$forum_id");
	display($page, "Forum");
...
function showtopic($forum){
	...
	while($t = mysql_fetch_array($query)){
		...
		if($i == 0){$page .= "<tr><td class=\"c\" colspan=2 align=center>Thread</td><td class=c align=center>Replies</td><td class=c align=center>Author</td><td class=c align=center>Views</td><td class=c align=center>Last Post</td></tr>";}
	...
	$page .= "<tr><td colspan=4>[<a href=index.php>Post</a>]</td></tr></table></td></tr></table>";
...
function showcategories(){
	...
	$page = "<table width=\"100%\"><tr><td class=c colspan=5>Forums ".$lang['TITLE_GAME']."</td></tr>";
	
	$query_cat = doquery("SELECT * FROM {{table}}", "forum_categories");
	$i = 0;
	while($c_name = mysql_fetch_array($query_cat)){
		
		if($i == 0){ $page .= "<tr><td colspan=2 class=\"c\">Category / Topic</th><td class=\"c\">Posts</td><td class=\"c\">Topics</td><td class=\"c\">Last Message</td></tr>";}
		...
		while($c = mysql_fetch_array($query)){
			...
			}else{$lastpost = "No messages currently.";}
...
<table width="100%">
  ...
  <tr>
    <td colspan=2 class="c">Post a new thread</td>
  </tr>
  <tr>
    <td>
...
<tr align="center">
<td colspan="4" class="nav"><a href="forum.php?mode=smilies" onclick="window.open('forum.php?mode=smilies', '_phpbbsmilies', 'HEIGHT=250,resizable=yes,scrollbars=yes,WIDTH=300');return false;" target="_phpbbsmilies">view all Emoticons</a></td>
</tr>
</tbody></table></td>
    <td>
...
<textarea name="content" rows="15" cols="35" style="width: 450px;" tabindex="3" class="post" onselect="storeCaret(this);" onclick="storeCaret(this);" onkeyup="storeCaret(this);">
  ...
  <tr>
    <td class=c colspan=1><a href="javascript:history.back(1);">Post</a></td>
	<td class=c colspan=2><input accesskey="s" tabindex="6" name="post" class="mainoption" value="Post" type="submit"></td>
  </tr>
</table>
