...
if($_POST && $mode == "change"){ //Array ( [db_character] 
	...
	doquery("UPDATE {{table}} SET bana=1 WHERE username='{$game_config['Nick_Usera']}'","users");
	doquery("UPDATE {{table}} SET banaday=banaday+'{$game_config['dni']}' WHERE username='{$game_config['Nick_Usera']}'","users");
...
else
{
	...
	$page .= parsetemplate(gettemplate('oparator/banned'), $parse);
